...
class KGCN(BaseModel):
    ...
    def save_entity_embeddings(self, file_path: str):
        """
        保存训练后的实体嵌入特征到.npy文件。

        :param file_path: 保存文件的路径
        """
        entity_embedding_layer = self.model.get_layer('entity_embedding')
        entity_embeddings = entity_embedding_layer.get_weights()[0]
        np.save(file_path, entity_embeddings)
        print(f'实体嵌入已保存到 {file_path}')