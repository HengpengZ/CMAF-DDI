...
def read_kg(file_path: str, entity_vocab: dict, relation_vocab: dict, neighbor_sample_size: int):
    ...
    for entity_id in range(n_entity):
        ...
        if n_neighbor == 0:
            # 处理没有邻居的实体，可以选择跳过或使用默认值
            # 这里使用自身作为默认邻居
            adj_entity[entity_id] = entity_id
            adj_relation[entity_id] = 85  # 使用-1表示没有关系
            continue

        sample_size = min(neighbor_sample_size, n_neighbor)
        ...
        adj_entity[entity_id, :sample_size] = [all_neighbors[i][0] for i in sample_indices]
        adj_relation[entity_id, :sample_size] = [all_neighbors[i][1] for i in sample_indices]
        if sample_size < neighbor_sample_size:
            adj_entity[entity_id, sample_size:] = entity_id  # 填充默认邻居
            adj_relation[entity_id, sample_size:] = 85  # 填充默认关系
