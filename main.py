...
def parse_SKGDDI_args():
    parser = argparse.ArgumentParser(description="Run CMAF.")
    ...
    parser.add_argument('--graph_embedding_file', nargs='?',
                        help='Input data path.')
    parser.add_argument('--entity_embedding_file', nargs='?',
                        help='Input data path.')
    parser.add_argument('--protein_embedding_file', nargs='?',
                        help='Input data path.')
    ...
    parser.add_argument('--pretrain_model_path', nargs='?', default='trained_model/models.pth',
                        help='Path of stored models.')

    parser.add_argument('--DDI_batch_size', type=int, default=2048,
                        help='DDI batch size.')
    parser.add_argument('--DDI_evaluate_size', type=int, default=8192,
                        help='KG batch size.')
    ...
    parser.add_argument('--structure_dim', type=int, default=300,
                        help='structure_dim')
    parser.add_argument('--protein_dim', type=int, default=320,
                        help='protein_dim')
    parser.add_argument('--pre_entity_dim', type=int, default=400,
                        help='pre_entity_dim')
    parser.add_argument('--smile_entity_dim', type=int, default=100,
                        help='pre_smile_dim')
    parser.add_argument('--feature_fusion', nargs='?', default='CMAF',
                        help='feature fusion type: concat / sum / CMAF')

    args = parser.parse_args()

    save_dir = 'trained_model/CMAF/epoch_{}/{}/all_entitydim{}_feature{}_lr{}_muti_type{}/'.format(args.n_epoch,
...
def train(args):
    ...
    # initialize data
    data = DataLoaderCMAF(args, logging)
    ...
    protein_pre_embed=torch.tensor(data.protein_pre_embed).to(device)
    smile_pre_embed=torch.tensor(data.smile_pre_embed).to(device)
    ...
    for i in range(5):
        ...
        # construct models & optimizer
        model = cmafddiModel(args, entity_pre_embed, structure_pre_embed,protein_pre_embed,smile_pre_embed)
        ...
        for epoch in range(1, args.n_epoch + 1):
            ...
            if (epoch % args.evaluate_every) == 0:
                ...
                if index == len(acc_list) - 1:
                    ...
                    logging.info('Save models on epoch {:04d}!'.format(epoch))
                    best_epoch = epoch
